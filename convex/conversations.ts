...
export const get = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!currentUser) {
      throw new ConvexError("User not found 6");
    }
    ...
    if (!conversationMemberships) {
      throw new ConvexError("User not in any conversation");
    }

    const conversations = await Promise.all(
      await conversationMemberships.map(async (membership) => {
        const conversation = await ctx.db.get(membership.conversationId);

        if (!conversation) {
          throw new ConvexError("Conversation not foundsss");
        }
